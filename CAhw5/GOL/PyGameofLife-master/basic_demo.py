...
def survival(x, y):
    ...
    num_neighbours = np.sum(
        universe[x - 1:x + 2, y - 1:y + 2]) - universe[x, y]
...
im_ani = animation.ArtistAnimation(
    fig, ims, interval=700, repeat_delay=1000, blit=True)
plt.show()
...
im_ani.save(
    'Cbeacon.mp4', writer=FFwriter,
    dpi=100)  #, writer=animation.FFMpegWriter())
